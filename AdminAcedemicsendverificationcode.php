...
$ace = Database::search("SELECT * FROM `academic_officer` WHERE `id`='".$id."'");
$na = $ace->num_rows;
if($na == 1){
$fs = $ace->fetch_assoc();
$e = $fs["email"];
...
Database::iud("UPDATE `academic_officer` SET `password`='".$upas."' WHERE `id`='".$id."'");
Database::iud("UPDATE `academic_officer` SET `username`='".$uname."' WHERE `id`='".$id."'");
Database::iud("UPDATE `academic_officer` SET `invitationSend`='2' WHERE `id`='".$id."'");
